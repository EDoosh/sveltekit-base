<script lang="ts">
	...
	async function getLeetSpeak() {
		result = ["loading"];
		result = await fetcher<string, Errors>("example", () =>
			fetch("/api/example", {
				method: "POST",
				headers: {
...
<div class="grid h-screen w-screen place-items-center bg-gray-950">
	<div
		class="flex max-w-2xl flex-col items-center rounded-xl bg-gray-900 px-12 py-8 shadow-lg"
	>
		...
		<p class="mt-2 self-end text-sm text-slate-500">
			Have fun coming up with a name for that one.
		</p>
		...
		<ul class="mt-2 list-disc text-lg text-gray-300">
			...
			<li class="ml-8 line-through">
				Receiving content from those endpoints in a safe way
			</li>
		...
		<div class="flex flex-row gap-8 mt-8">
			<input
				type="text"
				class="grow px-4 py-2 rounded-md bg-gray-800 text-gray-300 focus:outline-none active:outline-none focus:ring-2 focus:ring-blue-300"
				bind:value={text}
			/>

			<button
				class="px-8 py-2 rounded-md bg-gray-800 text-gray-300 hover:bg-gray-600 transition-colors duration-75 focus:outline-none active:outline-none focus:ring-2 focus:ring-blue-300"
				on:click={getLeetSpeak}
			>
				Leet-ify!
			</button>
		</div>
		...
		{:else if result[0] === "err"}
			<p class="mt-8 text-center text-gray-300">
				Error: {result[1]}{result[2] ? " | " + JSON.stringify(result[2]) : ""}
			</p>
		{/if}
	</div>
</div>
