...
describe("i18n tests", () => {
	...
	const proxy = get(
		createI18n<I18nKeys, typeof languages>(language, languages)
	);
	...
	it("allows valid nested function with args accesses", () => {
		expect(proxy.channel.MessagePlaceholder({ channel: "#hi" })).toBe(
			"Message #hi"
		);
	});
	...
	it("allows invalid root-level function with args accesses", () => {
		// @ts-expect-error
		expect(proxy.thisDoesntExist({ channel: "#hi" })).toBe(
			"TNF for 'thisDoesntExist?'"
		);
	});

	it("allows invalid nested string with args accesses", () => {
		// @ts-expect-error
		expect(String(proxy.channel.thisDoesntExist)).toBe(
	...
	it("allows multi-invalid nested function with args accesses", () => {
		expect(
			// @ts-expect-error
			proxy.channel.thisDoesntExist.neitherDoesThis({ channel: "#hi" })
		).toBe("TNF for 'channel.thisDoesntExist?.neitherDoesThis'");
	});
	...
	it("allows nesting past valid string with args accesses", () => {
		expect(
			// @ts-expect-error
			proxy.channel.MessagePlaceholder.thisDoesntExist({ channel: "#hi" })
		).toBe("TNF for 'channel.MessagePlaceholder!.thisDoesntExist'");
	});

	it("allows multi-nesting past valid string", () => {
		expect(
			// @ts-expect-error
			String(proxy.channel.MessagePlaceholder.thisDoesntExist.neitherDoesThis)
		).toBe(
	...
	it("allows multi-nesting past valid string with args accesses", () => {
		expect(
			// @ts-expect-error
			proxy.channel.MessagePlaceholder.thisDoesntExist.neitherDoesThis({
				channel: "#hi"
			})
		).toBe(
			"TNF for 'channel.MessagePlaceholder!.thisDoesntExist.neitherDoesThis'"
	...
	it("allows for .get access on root level", () => {
		...
		expect(proxy._get(["channel.MessagePlaceholder", { channel: "#hi" }])).toBe(
			"Message #hi"
		);
	});
	it("has a .get that doesn't allow invalid accesses", () => {
		...
		// @ts-expect-error
		expect(proxy._get(["this.doesntwork", { channel: "#hi" }])).toBe(
			"TNF for 'this?.doesntwork'"
		);
		// @ts-expect-error
		expect(proxy._get(["channel.MessagePlaceholder", { this: "#hi" }])).toBe(
			"Message {channel}"
		);
		// @ts-expect-error
